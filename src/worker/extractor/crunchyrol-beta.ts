...
// Types previously imported from './types' (file no longer exists)
type GetSeriesData = any
type GetEpisodesData = any
type GetEpisodesMeta = any
interface SearchData { type: string; items: CrunchyrollSerie[] }
type SearchMeta = any
...
export const handleToUrl = (handle: { id: string }) => `https://www.crunchyroll.com/series/${handle.id}`
...
export const fetchToken = async ({ fetch }: { fetch: typeof globalThis.fetch }) =>
...
const getToken = async ({ fetch }: { fetch: typeof globalThis.fetch }) => {
...
const getSeries = async (mediaId: string, { fetch }: { fetch: typeof globalThis.fetch }) =>
...
const _getSeason = async (mediaId: string, { fetch }: { fetch: typeof globalThis.fetch }) =>
...
const getSeasons = async (mediaId: string, { fetch }: { fetch: typeof globalThis.fetch }) =>
  fetch(`https://www.crunchyroll.com/content/v2/cms/series/${getMediaIdParts(mediaId).serieId}/seasons?force_locale=&preferred_audio_language=ja-JP&locale=en-US`, {
    "headers": {
      "accept": "application/json, text/plain, */*",
      "authorization": `Bearer ${(await getToken({ fetch })).access_token}`,
...
const getEpisodes = async (mediaId: string, { fetch }: { fetch: typeof globalThis.fetch }) =>
  fetch(`https://www.crunchyroll.com/content/v2/cms/seasons/${mediaId}/episodes?preferred_audio_language=ja-JP&locale=en-US`, {
    ...
    .then(res => res.data.map((episodeData: any) => crunchyrollEpisodeToScannarrEpisode(mediaId, episodeData)))

const getEpisode = async (mediaId: string, episodeId: string, { fetch }: { fetch: typeof globalThis.fetch }) =>
  fetch(`https://www.crunchyroll.com/content/v2/cms/objects/${episodeId}?ratings=true&locale=en-US`, {
    ...
    .then(res => res.data[0] ? crunchyrollEpisodeToScannarrEpisode(mediaId, res.data[0] as any) : undefined)

const search = async (query: string, { fetch }: { fetch: typeof globalThis.fetch }) =>
  // 100 episodes
...
const crunchyrollSerieToScannarrMedia = (serie: CrunchyrollSerie): GQLMedia => ({
  _id: crypto.randomUUID(),
  uri: toUri({ origin, id: serie.id }),
  origin,
  id: serie.id,
  url: `https://www.crunchyroll.com/series/${serie.id}`,
  handles: [],
  averageScore: Number(serie.rating.average) / 5,
  covers: (serie.images.poster_tall as any).at(-1)?.source
    ? [{ url: (serie.images.poster_tall as any).at(-1).source }]
    : [],
  descriptions:
    serie.description.length
      ? [{ language: 'en', description: serie.description }]
      : [],
  shortDescriptions:
    serie.description.length
      ? [{ language: 'en', shortDescription: serie.description }]
      : [],
  titles: [
    { language: 'en', title: serie.title }
  ],
  banners: [],
  episodes: [],
  trailers: [],
...
const crunchyrollSeasonToScannarrMedia = (serie: any): GQLMedia => ({
  _id: crypto.randomUUID(),
  uri: toUri({ origin, id: `${serie.series_id}-${serie.id}` }),
  origin,
...
const crunchyrollEpisodeToScannarrEpisode = (mediaId: string, episode: CrunchyrollEpisode) => ({
  _id: crypto.randomUUID(),
  uri: toUri({ origin, id: `${mediaId}-${episode.id}` }),
  origin,
  ...
  shortDescriptions: [],
  titles: [
    { language: 'en', title: episode.title }
  ],
  thumbnails: [],
  playback:
    (episode as any).external_id
      ? {
        type: 'IFRAME',
        origin,
        url: `https://www.crunchyroll.com/affiliate_iframeplayer?${
          new URLSearchParams({
            aff: 'af-44915-aeey',
            media_id: (episode as any).external_id.replace('EPI.', ''),
            video_format: '0',
            video_quality: '0',
            auto_play: '0'
          })
      ...
      : undefined
} satisfies GQLEpisode & { playback?: any })

const searchAnime = async (title: string, { fetch }: { fetch: typeof globalThis.fetch }) =>
...
const getFirstMatchingResult = async (resultPromises: { origin: string, data: Promise<{ Media: GQLMedia | null }> }[], func: (data: { Media: GQLMedia | null }) => boolean) => {
...
export const resolvers: Resolvers = {
  ...
  Media: {
    episodes: async (parent, _, ctx: ExtractorServerContext) => {
      const _origin = parent.origin
      const id = parent.id
      // console.log('Crunchyroll Media.Episode called with ', parent, id, _origin)
      if (_origin !== origin ) return undefined
      const { seasonId } = getMediaIdParts(id)
      const seasons = await getSeasons(id, { fetch: ctx.fetch })
      // console.log('Crunchyroll Media seasons ', id, seasonId, seasons)
      const season =
        seasonId
          ? seasons.data.find(({ id }: any) => id === seasonId)
          : undefined
      if (!season) return undefined
      const res = await getEpisodes(season.id, { fetch: ctx.fetch })
      // console.log('Crunchyroll Media.Episode called with ', parent, id, _origin, season.data[0]?.id, res)
