...
interface CrunchyrollSeries {
  ...
  description: string
  images: {
    poster_tall?: { source: string; width: number; height: number }[][]
    poster_wide?: { source: string; width: number; height: number }[][]
  }
  series_metadata?: {
...
const normalizeSeries = (series: CrunchyrollSeries): GQLMedia => {
  ...
  const posterUrl = series.images?.poster_tall?.at(-1)?.at(-1)?.source
  ...
  return {
    ...
    banners: series.images?.poster_wide?.at(-1)?.at(-1)?.source
      ? [{ url: series.images.poster_wide.at(-1)!.at(-1)!.source }]
      : [],
    episodes: [],
    trailers: [],
...
const normalizeEpisode = (episode: CrunchyrollEpisode, mediaUri: string): GQLEpisode => {
  ...
  const thumbnailUrl = episode.images?.thumbnail?.at(-1)?.at(-1)?.source
