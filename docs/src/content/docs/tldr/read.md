...
| `POPULARITY` first, `POPULARITY_DESC` last | `media/index.ts:156-160` against `media/schema.gql:425-426` | read the comparators, not the names: `POPULARITY` sorts descending and `POPULARITY_DESC` ascending, because AniList's upstream enum means the opposite and stub sends it that way |
